...
public class GetAccStatService implements IGetAccStatService {
	...
	private String geUrlForJSON(String callUrl) {

		log.info(this.getClass().getName() + " getUrlForJSON Start ! ");
		
		log.info("Requete URL : " + callUrl);
		...
		// ssl 적용된 사이트일 경우, 데이터 증명을 위해 사용함 
		HostnameVerifier allHostsValid = new HostnameVerifier() {
			@Override
			public boolean verify(String hostsname, SSLSession session) {
				return true;
			}
		};
		...
		try {
			...
			if(urlConn != null && urlConn.getInputStream()  == null ) {
				...
				// 주어진 문자 입력 스트림 inputStreem에 대해 기본 크기의 버퍼를 갖는 객체를 생성 
	...
	/**
	 * 교통사고 정보 가져오기
	 * @param pDTO 교통사고 정보 가져오기 위한 파라미터 들 
	...
	public Map<String, Object> getAccStatForJSON(AccStatDTO pDTO) throws Exception{
		...
		// String 변수의 문자열을 json 형태의 데이터 구조로 변경하기 위한객체를 메모리에 올림
		...
		//JSON 배열에저장된 데이터를 List<AccStatDTO> 구조로 변경하기 위해 메모리에 올림
		...
		for (int i = 0; i < resArr.size(); i++) {
			...
			rDTO = new AccStatDTO();// 데이터 저정을 위해 메모리에 올림 
	...
	private String getUrlForJSON(String nvl) {
		// TODO Auto-generated method stub
		return null;
	}
	@Override
	public Map<String, Object> getAccStatNightJson(AccStatDTO pDTO) throws Exception {
		// TODO Auto-generated method stub
		return null;
	}
